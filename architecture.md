...
### `SHOULD` Hide complexity
We reduce the complexity of the interfaces so that they can easily be used by the consumers as needed. 

The design of an API must follow the principle of [information hiding](https://en.wikipedia.org/wiki/Information_hiding). APIs **must not** transport the complexity of the application behind the API. This means it is relevant that an API hides implementation details. An API should be understandable and intuitive for humans that are not very familiar with the API's business domain. E.g. one should not be able to recognize if an API is provided by a software system built with ABAP, Java or .Net.

### `SHOULD` We build tolerant dependencies
When building dependencies between teams and applications, we focus on loose coupling. We implement [tolerant readers](https://martinfowler.com/bliki/TolerantReader.html) and we follow Postel's law:

>Be conservative in what you do, be liberal in what you accept from others

New versions of a dependency (API) should not be introduced unless there is no other way to satisfy a requirement. In case if breaking changes, another major version of the API must be introduced and maintained. APIs should not have more than two concurrent supported versions at any time. 
Consumers should not be forced to use one of these versions.  
...
### `SHOULD` Design API first

In a nutshell API First requires two aspects:

-   define APIs first, before coding its implementation, using a standardized specification language
...
### `SHOULD` Understand that APIs are (part of) the product

As a company we want to deliver products to our (internal and external) customers which can be consumed as a service.

Software products provide their functionality via (public) APIs; hence, the design of our APIs should be based on the API as a product principle:
...
### `SHOULD` APIs speak the business domain language
APIs must be consumer oriented and developer friendly. We design and build APIs in the language which is used by SBB, because that is how our business partners and agile teams understand it's context and usage.
...
* **Type G** (Generic): The API uses a very product specific language which does NOT fit our domain language (ofthen the case at very technical and generic APIs of MUST software), or it is very complex and requires a deep understanding of the software behind the API.

**Type S** applications can directly [publish](https://schweizerischebundesbahnen.github.io/api-principles/publication/) the API of the _of the shelf_ software over the [API Management](https://confluence.sbb.ch/x/noj5R) (_internal link_) infrastructure. Whereas **Type G** applications `MUST` write an own API which exposes the data and functionality of the application in the SBB's domain language, which is being understood by all the teams within the company (also outside of the company if it is a public API). We usually write **Type G** APIs using the [facade pattern](https://en.wikipedia.org/wiki/Facade_pattern).

#### When do we use of the shelf APIs?
{: .no_toc }

Now let's imagine that application A is using _of the shelf_ software and needs data and functions from application B, which is also using _of the shelf_ software and provides an API. _Of the shelf_ software usually provides integrated solutions for connecting to APIs provided by other _of the shelf_ software. In this case, we use these out-of-the-box integrations to connect application A with application B.

If explicit domain logic from the domain of application B has to be built on the consumer side (application A) in order to connect to the API provided by _of the shelf_ software, it is a sign that this logic should be built within the domain of application B in the form of a facade.

Let's take the Graph API from AzureAD versus the SBB specific Employee API as an example. In most cases, _of the shelf_ software provides native integrations with AzureAD in their system. If the connection can be made purely configurable, the AzureAD API should also be used. However, if SBB specific domain logic from HR is required, the Employee API should be used and extended, if necessary.
